...
class Qwen2Attention(nn.Module):
    ...
    def forward(
        ...
        # Input Projection, generating Q, K, V
        ...
        if past_key_value is not None:
            # past_key_value: Tuple[(batch_size, past_kv_seq_len, num_key_value_heads, head_dim),
            #                       (batch_size, past_kv_seq_len, num_key_value_heads, head_dim)]
            kv_seq_len += past_key_value[0].shape[1]

        # RoPE
        # cos, sin: Tuple[(1, q_len, 1, head_dim), (1, q_len, 1, head_dim)]
        cos, sin = _get_rotary_pos_emb(self.rope_theta,
                                       position_ids, 
                                       self.head_dim)
        # Q,K: [bsz, len, num_heads, head_dim]
        ...
        query_states = query_states.transpose(1, 2) # [bsz, num_heads, len, head_dim]
        key_states = key_states.permute([0, 2, 3, 1]) # [bsz, num_heads, head_dim, len]
        value_states = value_states.transpose(1, 2) # [bsz, num_heads, len, head_dim]
        ...
        # return output and past_key_value
